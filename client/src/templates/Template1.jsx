...
function Template1({ data, isPreview = false }) {
  ...
  const headerFont = 'Georgia, serif'

  return (
    <div className={`${isPreview ? 'max-w-4xl mx-auto shadow-lg' : ''} bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100`} style={{fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial'}}>
      <div className="flex min-h-screen border border-transparent">
        {/* Left Sidebar - Contact Info & Skills */}
        <aside className="w-1/3 bg-gray-50 dark:bg-gray-800 p-8">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-1" style={{fontFamily: headerFont}}>{personalInfo.firstName} {personalInfo.lastName}</h1>
          ...
          {personalInfo.summary && (
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{personalInfo.summary}</p>
            </div>
          )}
        </aside>
        ...
        <main className="flex-1 p-8">
          {experience && experience.length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 uppercase tracking-wide pb-1" style={{borderBottom: '2px solid rgba(209,213,219,1)'}}>Experience</h2>
          ...
          {education && education.length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 uppercase tracking-wide pb-1" style={{borderBottom: '2px solid rgba(209,213,219,1)'}}>Education</h2>
              <div className="space-y-4">
                {education.map((ed,i) => (
                  <div key={i} className="mb-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-semibold text-gray-900 dark:text-white">{ed.degree}{ed.field ? ` — ${ed.field}` : ''}</h3>
          ...
          {projects && projects.length > 0 && (
            <section>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4 uppercase tracking-wide pb-1" style={{borderBottom: '2px solid rgba(209,213,219,1)'}}>Projects</h2>
              <div className="space-y-4">
                {projects.map((p,i) => (
                  <div key={i} className="mb-3">
                    <div className="flex justify-between items-start">
